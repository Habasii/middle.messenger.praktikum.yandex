function validationConditions(code:string, value:string){
  ...
  switch(code){
    ...
    case 'email':
      ...
      break;
    case 'password':
      if(!/^.{8,40}$/.test(value)) error = "От 8 до 40 символов, обязательно хотя бы одна заглавная буква и цифра";
      else if(!/[A-ZА-Я]/gi.test(value)) error = 'Обязательно хотя бы одна заглавная буква и цифра';
      else if(!/\d/gi.test(value)) error = 'Обязательно хотя бы одна заглавная буква и цифра';
      break;
    case 'phone':
      if(!/^(\+{1}\d{9,14})|(\d{10,15})$/.test(value)) error = "От 10 до 15 символов, состоит из цифр, может начинается с плюса.";
      break;
    case 'message':
